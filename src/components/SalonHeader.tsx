    ...
    >
      ...
      {open && (
        <div className="md:hidden mt-2 mx-3 rounded-3xl border border-white/30 bg-background/90 shadow-lg backdrop-blur-xl">
          <ul className="flex flex-col pimport { useEffect, useState } from "react";
import { Menu, X } from "lucide-react";
import { Link } from "@tanstack/react-router";
import { Button } from "@/components/ui/button";
import { salon, whatsappUrl } from "@/lib/salon";
...
export function SalonHeader() {
  ...
  // Shared dynamic class for text links
  const linkTextClass = scrolled
    ? "text-foreground/80 hover:text-foreground" // Theme brown on glass pill
    : "text-amber-50/90 hover:text-white drop-shadow-[0_1px_4px_rgba(0,0,0,0.4)]"; // Soft cream/white over dark hair
          ...
          <img
            ...
            className={`h-9 w-auto transition-all duration-500 md:h-10 ${
              scrolled
                ...
                // Forces logo to pure white/cream when over the dark background if it's natively brown
            }`}
          />
          <span className="sr-only">{salon.fullName}</span>
        </Link>
